...
while ($row1Data = oci_fetch_array ($rs1Data, OCI_BOTH)) {
  ...
  while ($deepMaterial > $lpDt) {
    $lpDt++;
    $SqlData = "select
                  MGTAPPS.pkg_yarn_valuation.fGetCylSysID_byLvl($P_CYL_SYS_ID_DTL,$deepMaterial,$lpDt)   GET_DATA
                from dual";
    ...
    if ($deepMaterial > $lpDt){
      ...
      if($stsDtl===1){
        ...
        while ($deepMaterial > $lpDt2) {
          ...
          $SqlData = "select
                            MGTAPPS.pkg_yarn_valuation.fGetCylSysID_byLvl($P_CYL_SYS_ID_DTL,$deepMaterial,$lpDt2)   GET_DATA
                          from dual";
          $CylSysId_dt =  getData($conn,$SqlData);
          $dtVal = "";
          $dtVal =  getDtVal( $conn
