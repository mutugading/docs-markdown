...
$isUseRmBo="";
if(isset($_GET['CYL_SHADE_NAME_S'])){$CYL_SHADE_NAME_S=$_GET['CYL_SHADE_NAME_S'];
  $SqlGet	= "SELECT REPLACE (GET_DATA, ',', ''',''') GET_DATA
             FROM (
              SELECT '(''$CYL_SHADE_NAME_S'')' GET_DATA FROM DUAL
             )";
...
<?php
$excelNm = "Summary List Product Colour Base.Xls";
header("Content-type: application/vnd-ms-excel");
header("Content-Disposition: attachment; filename=$excelNm");
echo "<table>";
echo "<tr>";
echo "<td>Report : Summary List Product Colour $CYL_TYPE_S </td>";
echo "</tr>";
echo "</table>";
?>
<table border="1">
  <tr>
    ...
    <td>
  <?php
    if ($P_FORM_NAME==="CST_T_001" || $P_FORM_NAME==="CST_T_006"){
      echo  "2)<br>Customer";
    } else {
      echo  "2)<br>Type";
    }
  ?>
    ...
    <td>
      <?php
        if ($P_FORM_NAME==="CST_T_003" || $P_FORM_NAME==="CST_T_002"){
          echo  "3)<br>Customer";
        } else {
          echo  "3)<br>Shade Name";
        }
      ?>
    </td><!-- 3 -->
    <td>
...
<?php
  ...
  //echo "<tr><td>SqlView $SqlView</td></tr>";
  ...
  while ($rowRsView = oci_fetch_array ($rsView, OCI_BOTH)) {
    ...
    //val data 27
        ...
        //cek is Dt RM 55
    		...
    		if ($isDt_RM55==="Y"){
    				// if ($USER_NAME==="1949"){
    				// 	echo "isDt_RM55 $isDt_RM55 ".$rowRsView['CYL_SYS_ID']."<br>";
    				// }
    				$SqlData =  "
...
<tr>
    <td>
      <?php echo $rowRsView['CYL_LEFT_NO']; ?> <!-- 0 no -->
    </td><!-- 0 -->
    <td>
      <?php echo $rowRsView['CYL_SHADE_NAME']; ?> <!-- 0 no -->
    </td><!-- 0 -->
    <td>
    ...
    <td <?php echo "style='font-size: 18px;background-color:$ClrView24'"; ?> >
      <b><?php echo $valDt24; ?></b><!--24<br>Chip Rate -->
    </td><!-- 7 -->
