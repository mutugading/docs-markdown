<?php
	...
	if ($CMY_PRODUCTION_S!=="NULL") { $WHERE_PARAM = " $WHERE_PARAM and CMY_PRODUCTION = '$CMY_PRODUCTION_S' ";	}
?>
...
<?php
		...
		$sqlSlct = " SELECT DISTINCT CYL_TYPE from mgtapps.cst_yarn_left where CYL_IS_PROJECT = 'Y' ";
		if ($USER_NAME === "1949"){
				//echo $sqlSlct;
		}
...
<!-- Left No -->
		<td <?php echo getStyle($font,"center"); ?>>
			...
			<input type="TEXT" id="CYL_LEFT_NO" name="CYL_LEFT_NO" style="width:75px;height:20px;"
				value="<?php echo $CYL_LEFT_NO_S; ?>"2697o
			>&nbsp<button onclick="Process('VIEW_DATA')">Find</button>
		</td>
<!-- Left No -->
<!-- Customer -->
		<td <?php echo getStyle($font,"center"); ?>>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"CUSTOMER",$WHERE_PARAM);
	//echo "$sqlSlct </br>";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<!-- Product Name -->
		<td <?php echo getStyle($font,"center"); ?>>
			Product Name  </br>
<?php
		$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"CYL_PRODUCT_QUALITY",$WHERE_PARAM)." order by CYL_PRODUCT_QUALITY ";
		$rsSlct = oci_parse($conn,$sqlSlct);
		//echo "$sqlSlct </br>";
		oci_execute ($rsSlct);
?>
...
<!-- Shade Name - Code -->
		<td <?php echo getStyle($font,"center"); ?>>
			Shade Name </br>
<?php
		//$sqlSlct = sqlSlctShadeNm($CYL_TYPE_S);
		//$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"CYL_SHADE_NAME");
		$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"CYL_SHADE_NAME",$WHERE_PARAM)." order by CYL_SHADE_NAME ";
		//echo "$sqlSlct </br>";
		$rsSlct = oci_parse($conn,$sqlSlct);
		oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"DENIER",$WHERE_PARAM)." order by to_number(DENIER) ";
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"FILAMENT",$WHERE_PARAM)." order by to_number(FILAMENT) ";
	//echo "sqlSlct $sqlSlct</br>";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"INTERMINGLING",$WHERE_PARAM)." order by INTERMINGLING ";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"HEATSET",$WHERE_PARAM)." order by HEATSET ";
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"CROSS_SECTION",$WHERE_PARAM)." order by CROSS_SECTION ";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBasePrjct($conn,$sqlData,$CYL_TYPE_S,"LUSTURE",$WHERE_PARAM);
	//echo $sqlSlct;
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	"  SELECT DISTINCT CMY_PRODUCTION
  ...
  ORDER BY CMY_PRODUCTION ";
	//echo $sqlSlct;
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
<select name="CMY_PRODUCTION" id="CMY_PRODUCTION" onchange="Process('VIEW_DATA')" <?php echo $styleSlct; ?> >
			<option value="NULL">NULL</option>
<?php
		while ($rowRsSlct = oci_fetch_array ($rsSlct, OCI_BOTH)) {
?>
			<option value="<?php echo $rowRsSlct['CMY_PRODUCTION']; ?>"
				<?php  if ($CMY_PRODUCTION_S=== $rowRsSlct['CMY_PRODUCTION']) {echo "selected"; } ?>
			>
				<b><font color='red' ><?php echo $rowRsSlct['CMY_PRODUCTION']; ?></font></b>
			</option>
<?php
		}
?>
