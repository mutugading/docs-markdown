...
<!-- Customer -->
		<td align="center">
			<b>Customer</b> </br>
<?php
	$sqlSlct = sqlSlctCust($sqlData,"SELECT DISTINCT cmcd_name CUSTOMER ")." and cmcd_name = 'GUIDELINES' order by cmcd_name ";
	//echo "$sqlSlct </br>";
...
<?php
		...
		$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"CYL_TYPE",$WHERE_PARAM)." where CYL_TYPE <> 'POY' order by CYL_TYPE ";
		//echo $sqlSlct;
		$rsSlct = oci_parse($conn,$sqlSlct);
		oci_execute ($rsSlct);
?>
...
<?php
		...
		$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"CYL_SHADE_NAME",$WHERE_PARAM)." order by CYL_SHADE_NAME ";
		//echo $sqlSlct;
		$rsSlct = oci_parse($conn,$sqlSlct);
		oci_execute ($rsSlct);
?>
...
<!-- Denier -->
		<td  <?php echo getStyle($font,"center"); ?>>
			Denier </br>
<?php
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"DENIER",$WHERE_PARAM)." order by to_number(DENIER) ";
...
<!-- Filament -->
		<td  <?php echo getStyle($font,"center"); ?>>
			Filament </br>
<?php
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"FILAMENT",$WHERE_PARAM)." order by to_number(FILAMENT) ";
	//echo "sqlSlct $sqlSlct</br>";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<!-- Intermingling -->
		<td <?php echo getStyle($font,"center"); ?>>
			Intermingling </br>
<?php
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"INTERMINGLING",$WHERE_PARAM)." order by INTERMINGLING ";;
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<!-- Heatset -->
		<td <?php echo getStyle($font,"center"); ?>>
			Heatset </br>
<?php
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"HEATSET",$WHERE_PARAM)." order by HEATSET ";
...
<!-- Cross Section -->
		<td <?php echo getStyle($font,"center"); ?>>
			Cross Section </br>
<?php
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"CROSS_SECTION",$WHERE_PARAM)." order by CROSS_SECTION ";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct = sqlSlctCustBase($conn,$sqlData,$CUSTOMER_S,"LUSTURE",$WHERE_PARAM);
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
