...
<table style="width:80%" border="1" align="center">
	<tr>
		<td align="center" style="width:10%"></td><!--1-->
		<td align="center">Delpack name</td><!--1-->
		<td align="center" style="width:10%">Box Rate</td><!--1 Box Rate-->
...
?>
	<tr>
		...
		<td align="center"><?php echo //setNumber($conn,"Number",$boxRate_1,3);
														 			setDecimal($conn,$boxRate_1,3);
												?></td><!--1 Box Rate-->
		<td align="center"><?php echo setDecimal($conn,$bobinRate_1,3); ?></td><!--1 Bobin Rate-->
		...
		<td align="center"><?php echo setDecimal($conn,$DELPACKINGCOST_1,3); ?></td><!--5-->
		<td align="center"></td>
		<!--<td align="center"><?php echo setNumber($conn,"Number",$V1_1,2); ?></td>
		<td align="center"><?php echo setNumber($conn,"Number",$V2_1,2); ?></td>
		<td align="center"><?php echo setNumber($conn,"Number",$V3_1,2); ?></td>
		<td align="center"><?php echo setNumber($conn,"Number",$V4_1,2); ?></td>
...
<?php
	...
	$styleReadOnly = "style='height:20px; width:100px; background-color:D8D8D8' readonly";
?>
	<tr>
		<td>Simulate</td>
		<td align="center">
<?php
	$sqlSlct = "select * from mgtapps.cst_mst_box_bobin_cost
							where CMBBC_TYPE in (
								select MPD_VALUE from mgtapps.mst_param_Data where MPD_MPDK_KEY = 'PACKING PARAM SIMULATION COSTING'
							)
							order by CMBBC_TYPE";
	//echo $sqlSlct;
?>
			<select id="Delpack_name" name="Delpack_name" onchange="<?php echo $btnPrsSmltn; ?>" <?php echo $bgClrEntry; ?> >
...
<?php
	...
	if ($Delpack_name_S === "Choose Data"){
		$CMBBC_BOX_COST = 0; $CMBBC_BOBIN_COST = 0; $AX_WT_S=0; $NO_OF_BOBBINS_S=0; $BoxWeightNet_2=0; $IncrmntCost=0; $DelPackCost_2=0;
	} else {
		...
		while ($rowRsSlct = oci_fetch_array ($rsSlct, OCI_BOTH)) {
			if (!empty($rowRsSlct['CMBBC_BOX_COST'])){
				$CMBBC_BOX_COST = $rowRsSlct['CMBBC_BOX_COST'];
			}
			if (!empty($rowRsSlct['CMBBC_BOBIN_COST'])){
				$CMBBC_BOBIN_COST = $rowRsSlct['CMBBC_BOBIN_COST'];
			}
		}
			...
			if ($AX_WT_S!=="" && $NO_OF_BOBBINS_S !=="") {
				...
				if ((int)$GrossBoxWt2!==0){
					$existingRasio2 = $BoxWeightNet_2 / $GrossBoxWt2;
				} else $existingRasio2 = 0;

				$boxRate_2 = $CMBBC_BOX_COST;
				$bobinRate_2 = $CMBBC_BOBIN_COST;
				...
				if ((int)$BoxWeightNet_1!==0){
					$DelPackCost_1 = ($boxRate_1+($bobinRate_1*$NO_OF_BOBBINS_1))/$BoxWeightNet_1;
				}	else {$DelPackCost_1 =0;}

				//echo "BoxWeightNet_2 $BoxWeightNet_2</br>";
				if((int)$BoxWeightNet_2!==0){
					//echo "($boxRate_2+($bobinRate_2*$NO_OF_BOBBINS_2))/$BoxWeightNet_2 </br>";
					$DelPackCost_2 = ($boxRate_2+($bobinRate_2*$NO_OF_BOBBINS_2))/$BoxWeightNet_2;
					//echo "DelPackCost_2 $DelPackCost_2 </br>";
				} else {
					$DelPackCost_2 = 0;
				}
	...
	$keyPrssEntry = "Javascript: if (event.keyCode==13) Process('".str_replace(" PROCESS", "", $STSPRS)." PROCESS'); "//"Process('".str_replace(" PROCESS", "", $STSPRS)." PROCESS')"
	;

	//if ($Delpack_name_S!=="Choose Data"){
?>
		<td align='center'>
			<input type="text" name="CMBBC_BOX_COST" id="CMBBC_BOX_COST" value="<?php echo setDecimal($conn,$CMBBC_BOX_COST,3);?>" align="right"
				<?php echo $styleReadOnly;?>
			> <!-- Box Rate -->
		</td>
		<td align='center'>
			<input type="text" name="CMBBC_BOBIN_COST" id="CMBBC_BOBIN_COST" value="<?php echo setDecimal($conn,$CMBBC_BOBIN_COST,3); ?>" align="right"
			<?php echo "$styleReadOnly ";?> >
		</td><!-- Bobin Rate -->
		<td align='center'>
			<input type="Number" id="AX_WT" name="AX_WT" value="<?php echo setDecimal($conn,$AX_WT_S,3); ?>" <?php echo $bgClrEntry; ?>
						 onkeypress="<?php echo $keyPrssEntry; ?>"
			>
		</td><!-- AX-wt -->
		<td align='center'>
			<input type="Number" id="NO_OF_BOBBINS" name="NO_OF_BOBBINS" value="<?php echo setDecimal($conn,$NO_OF_BOBBINS_S,3); ?>" <?php echo $bgClrEntry; ?>
						 onkeypress="<?php echo $keyPrssEntry; ?>"
			>
		</td><!-- No of Bobbins -->
		<td align='center'>
			<?php echo setDecimal($conn,$BoxWeightNet_2,3); ?>
		</td><!-- Box weight (net) -->
		<td align="center">
			<?php echo setDecimal($conn,$DelPackCost_2,3); ?>
		</td><!-- Box weight (net) -->
		 <td align="center"><?php echo setDecimal($conn,$IncrmntCost,3); ?></td>
	</tr>
</table>
</br>
</br>
