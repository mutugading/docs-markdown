<script type='text/javascript'>
	...
	function Process(prs){
		...
		var CYL_SHADE_NAME_S ="<?php Print($CYL_SHADE_NAME_S); ?>";
		var vSTSPRS ="<?php Print($STSPRS); ?>";
		var CYL_SHADE_NAME_TMP ="<?php Print($CYL_SHADE_NAME_TMP); ?>";
		getParameter();
		//alert("CYL_SHADE_NAME_TMP " + CYL_SHADE_NAME_TMP);
		getParamSimulation();
		getParamPtySmltn();
		...
		if (prs==="VIEW_DATA"){
			if (CYL_SHADE_NAME_TMP==="UNSELECTED"){
				var selected = [];
				for (var option of document.getElementById('CYL_SHADE_NAME').options)
				{
						if (option.selected) {
								selected.push(option.value);
						}
				}
				document.getElementById('CYL_SHADE_NAME_TMP').value = selected;
			}
		} else {
			document.getElementById('CYL_SHADE_NAME_TMP').value = CYL_SHADE_NAME_TMP;
		}


		if (prs==="CLEAR_PARAM"){
			document.getElementById('CYL_SHADE_NAME_TMP').value = "UNSELECTED";
		}

		//alert('Param Selected '+document.getElementById('CYL_SHADE_NAME_S').value);
		if (document.getElementById('CYL_SHADE_NAME_S').value === "NULL"
			|| document.getElementById('CYL_SHADE_NAME_S').value !== CYL_SHADE_NAME_S	) {
		...
		//alert('Param Selected 2 '+document.getElementById('CYL_SHADE_NAME_S').value);
