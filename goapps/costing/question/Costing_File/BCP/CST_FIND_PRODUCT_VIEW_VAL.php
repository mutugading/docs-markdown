<?php
	$SqlView = "SELECT 	CMY_NAME
											,CYCCD_TOP_13_DATA_VALUE DENIER
											,CYCCD_TOP_16_DATA_VALUE FILAMENT
											,CYCCD_TOP_18_DATA_VALUE INTERMINGLING
											,CYCCD_TOP_49_DATA_VALUE HEATSET
											,CYCCD_TOP_17_DATA_VALUE CROSS_SECTION
											,CMY_LUSTURE
											,CYL_SYS_ID
											,CYL_SHADE_CODE
											,CYL_SHADE_NAME
											,CYL_LEFT_NO
											,CYL_TYPE
											,MGTAPPS.pkg_yarn_valuation_param.fGetValueData('$PERIOD_DATA_S','Fowarding Cost') Fowarding_Cost
             FROM mgtapps.cst_mst_yarn y,
                  mgtapps.cst_yarn_left l,
                  mgtapps.cst_yarn_calculation_cur_data cc
	...
	if ($FORM_NAME === "CST_2P1_0001"){
		$SqlView = "select 	CMY_NAME
								...
								from (
											SELECT 	CMY_NAME
															,CYCCD_TOP_13_DATA_VALUE
															,CYCCD_TOP_16_DATA_VALUE
															,CYCCD_TOP_18_DATA_VALUE
															,CYCCD_TOP_49_DATA_VALUE
															,CYCCD_TOP_17_DATA_VALUE
															,CMY_LUSTURE
															,CYL_SYS_ID
															,CYL_SHADE_CODE
															,CYL_SHADE_NAME
										...
										,(SELECT MPD_VALUE, MPD_SEQ_NO,CYL_LEFT_NO MPD_LEFT_NO
						          FROM MGTAPPS.MST_PARAM_DATA a,mgtapps.cst_yarn_left b
						         	WHERE mpd_mpdk_key = 'GUIDELINES SEQ'
											and MPD_VALUE = CYL_SYS_ID order by MPD_SEQ_NO
											) dt_seqno
 								WHERE dt_seqno.MPD_LEFT_NO(+) = dt.CYL_LEFT_NO ";
		// if ($USER_NAME === "1949" ) {
		 	//echo "$SqlView</br>";
		// }
	}
  ...
  $SqlViewWhere =
	" and CYCCD_PERIOD = replace('$PERIOD_DATA_S','-','')
		and CYL_SYS_ID in (
			select CYL_SYS_ID
			from mgtapps.cst_mst_orion_reff_Dtl a
			     ,mgtapps.cst_mst_orion_reff_hdr b
			     ,mgtapps.cst_yarn_left c
			where A.CMORD_CMORH_SYS_ID = CMORH_SYS_ID
			and  B.CMORH_PERIOD_YEAR = to_char(to_date('$PERIOD_DATA_S','YYYY-MM'),'YYYY')
			and  B.CMORH_PERIOD_MONTH = to_char(to_date('$PERIOD_DATA_S','YYYY-MM'),'MM')
			and CMORD_ITEM_CODE = CYL_ITEM_CODE
			and CMORD_ITEM_SHADE = CYL_SHADE_CODE
			and CMORD_SHADE_NAME = CYL_SHADE_NAME
			)
	";
  ...
  if ( $CMY_LUSTURE_S !=="NULL"){
	...
	if ( $DENIER_S !=="NULL"){
  	$SqlViewWhere = "$SqlViewWhere and CYCCD_TOP_13_DATA_VALUE = '$DENIER_S' ";
  }
  if ( $FILAMENT_S !=="NULL"){
  	$SqlViewWhere = "$SqlViewWhere and CYCCD_TOP_16_DATA_VALUE = '$FILAMENT_S' ";
  }
  if ( $INTERMINGLING_S !=="NULL"){
  	$SqlViewWhere = "$SqlViewWhere and CYCCD_TOP_18_DATA_VALUE = '$INTERMINGLING_S' ";
  }
  if ( $CROSS_SECTION_S !=="NULL"){
  	$SqlViewWhere = "$SqlViewWhere and CYCCD_TOP_17_DATA_VALUE = '$CROSS_SECTION_S' ";
  }
	if ( $HEATSET_S !=="NULL"){
		$SqlViewWhere = "$SqlViewWhere and CYCCD_TOP_49_DATA_VALUE = '$HEATSET_S' ";
	}
  if ( $CUSTOMER_S !=="NULL"){
  	$SqlViewWhere = "$SqlViewWhere and CYL_LEFT_NO in ( select distinct CYL_LEFT_NO  from mgtapps.CST_YARN_LEFT_CUST lc,mgtapps.CST_YARN_LEFT l,mgtapps.CST_MST_CUST_DATA cd
  ...
  $SqlCnt = "select count(-1) GET_DATA from ($SqlView)  ";
	...
	//echo "SqlCnt $SqlCnt </br> StsViewDt $StsViewDt </br>";	die();
	$totRows = $StsViewDt;
...
<?php
	...
	$sqlSlctVal =  getSlctVal($PERIOD_DATA_S);
...
//val data 27
		...
		if ($rowRsView['CYL_TYPE']==="MELANGE"){
			...
			if (!empty($rowRsView['FGETMBCOST'])){
				...
				$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.FGETMBCOST_RM('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
				//echo $sqlDtRm;
...
//val data 24
		...
		if ($rowRsView['CYL_TYPE']==="MELANGE"){
			...
			if (!empty($rowRsView['FGETCHIPRATE'])){
				...
				$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.fgetchiprate_rm('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
				//echo $sqlDtRm;
				$DtRm =  getData($conn,$sqlDtRm);
				$valDt24 = $DtRm;
			}
...
<?php } ?>
		...
		><!-- 6 -->
			<b><?php
							if (!empty($rowRsView['FGET_V4'])){ echo setNumber($conn,"Number",$rowRsView['FGET_V4'],4);  }
							//echo "test </br>";
				 ?>
			 </b>
		</td>
			...
			<?php
				if ($rowRsView['CYL_TYPE']==="MELANGE"){
					...
					if (!empty($rowRsView['FGETMBNAME'])){
						...
						$sqlMbNmRaw = "select MGTAPPS.pkg_yarn_valuation.fgetmbname_rm('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
		...
		<td>
			...
			if ($rowRsView['CYL_TYPE']==="MELANGE"){
				...
				if (!empty($rowRsView['FGETCHIP'])){
					...
					$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.fgetchip_rm('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
					$DtRm =  getData($conn,$sqlDtRm);
					echo $DtRm;
				}
			}
		...
		<td align="right">
			...
			if ($rowRsView['CYL_TYPE']==="MELANGE"){
				...
				if (!empty($rowRsView['FGETRPDOZ'])){
					...
					$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.FGETRPDOZ_rm('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
					$DtRm =  getData($conn,$sqlDtRm);
		...
		<td align="right">
			...
			if ($rowRsView['CYL_TYPE']==="MELANGE"){
				...
				if (!empty($rowRsView['FGETMBRATE'])){
					...
					$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.FGETMBRATE_RM('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
					$DtRm =  getData($conn,$sqlDtRm);
		...
		<td align="right">
			...
			if ($rowRsView['CYL_TYPE']==="MELANGE"){
				...
				if (!empty($rowRsView['FGETCNGOVRLST'])){
					...
					$sqlDtRm= "select MGTAPPS.pkg_yarn_valuation.FGETCNGOVRLST_RM('".$rowRsView['CYL_SYS_ID']."') GET_DATA from  dual";
					$DtRm =  getData($conn,$sqlDtRm);
					echo setNumber($conn,"Number",$DtRm,3);
				}
			}
		...
		<td align="right">
			<!-- 34 -->
			<?php  if (!empty($rowRsView['FOWARDING_COST'])){echo $rowRsView['FOWARDING_COST'];} ?>
		</td>
