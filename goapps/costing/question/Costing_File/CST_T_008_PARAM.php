...
<?php
	...
	if ($CMY_NAME_S!=="NULL") {
		$WHERE_PARAM = " $WHERE_PARAM and upper(CMY_NAME) like '%'||upper('$CMY_NAME_S')||'%' ";
		$WHERE_YARN_CC= " $WHERE_YARN_CC and upper(CMY_NAME) like '%'||upper('$CMY_NAME_S')||'%' ";
	}
	...
	if ($CMY_PRODUCTION_S!=="NULL") {
			$WHERE_PARAM = " $WHERE_PARAM and CMY_PRODUCTION = '$CMY_PRODUCTION_S' ";
			$WHERE_YARN_CC= " $WHERE_YARN_CC and CMY_PRODUCTION = '$CMY_PRODUCTION_S' ";
	}
	if ($CUSTOMER_S!=="NULL"){
	...
	if ($WHERE_YARN_CC!==""){
		$WHERE_YARN_CC =
		" AND CYL_TYPE in (
		 	select distinct CYL_TYPE
		 	from 	mgtapps.cst_yarn_calculation_cur a
						,mgtapps.cst_yarn_left b
						,mgtapps.cst_mst_yarn c
		 where a.CYCC_CYL_SYS_ID = CYL_SYS_ID and CMY_SYS_ID = CYL_CMY_SYS_ID $WHERE_YARN_CC)
		";
	}
	if ($CYL_LEFT_NO_S!==""){
	...
	//if ($USER_NAME==="1949"){echo "WHERE_YARN_CC $WHERE_YARN_CC";}
	$font = "13";
?>
...
<!-- Name -->
		<td <?php echo getStyle($font,"center"); ?>>
			Name </br>
			<input
				list="CMY_NAMES" id="CMY_NAME" name="CMY_NAME"
				style="width:300px;height:20px;"
				onchange="Process('VIEW_DATA')"
				onkeypress="Javascript: if (event.keyCode==13) Process('VIEW_DATA');"
				autocomplete="off"
				<?php
					if ($CMY_NAME_S!=="NULL") {
				?>
					value="<?php echo $CMY_NAME_S; ?>"
				<?php
					echo "readonly";
					}
				?>
			>
			<datalist id="CMY_NAMES">
	      <?php
	          $sqlSlct =
						"
						SELECT distinct CMY_NAME
						  FROM mgtapps.cst_mst_yarn y,
						       mgtapps.cst_yarn_left l
						 WHERE cyl_cmy_sys_id = cmy_sys_id
	      ...
	      ?>
	            <option value="<?php echo $rowRsSlct['CMY_NAME']; ?>">
	      <?php
	          }
	      ?>
	    </datalist>
			<?php
			  if ($CMY_NAME_S!=="NULL") {
			?>
			  <button onclick="Process('VIEW_DATA','CMY_NAMES','CLEAR')">clear</button>
			<?php
			  }
			?>
		</td>
