...
while ($row1Data = oci_fetch_array ($rs1Data, OCI_BOTH)) {
  ...
  for ($x = 1; $x <= $TotLpDt; $x++) {
    ...
    while ($deepMaterial > $lpDt) {
      ...
      //if ($row1Data['CYCRL_SEQ_NO']==="82"){ echo "getDtVal_RM55 $dtVal "; die();}
      //dtCek
      $dtCek = 0;
      if (is_numeric($dtVal)){
        $dtCek = $dtVal;
    ...
    //echo "CYCRL_SEQ_NO ".$row1Data['CYCRL_SEQ_NO'];die()
    //if ($row1Data['CYCRL_SEQ_NO']==="82"){ echo "Cek proses"; die();}
//Product
  }//disini
}
echo "</table>";
