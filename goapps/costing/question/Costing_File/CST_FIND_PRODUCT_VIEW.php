...
<table style="width:100%">
	<tr >
		<td  align="center">
		<input type="button" id="FIRSTREC" value="<<" onclick="FirstRecord()" >
		<input type="button" id="PREVREC" value="<" onclick="PrevRecord()" >
		...
		<input type="button" id="NEXTREC" value=">" onclick="NextRecord()" >
		<input type="button" id="LASTREC" value=">>" onclick="LastRecord()" >
		</td>
	</tr>
</table>
</div>
<div class="main" style="overflow-x:auto;" align="center">
<table style="width:240%" border="2">
	<tr>
...
//val data 27
		...
		//cek final product is SUPERBA
		// if($USER_NAME==="1949"){ echo "value DtRm $DtRm"; }
		if ($DtRm===""){
			//cek is Dt RM 55
			// if ($USER_NAME === "1949") {echo "Cek cek is Dt RM 55</br>";}
			...
			if ($isDt_RM55==="Y"){
					...
					if ($TotPoy_Rm55===1){
						...
						if ($isMulti_RM==="Y"){
							// if ($USER_NAME==="1949"){ echo "Multi RM<br>"; }
							$SqlMbDt =
							...
							// if ($USER_NAME==="1949"){ echo "Bukan Multi RM<br>"; }
							$sqlGet = "
							...
							if ($USER_NAME==="1949"){
								// echo "sqlGet $sqlGet<br>";
								// echo "isMultiRM_2 $isMultiRM_2 : DtRm_CYL_SYS_ID $DtRm_CYL_SYS_ID<br>";
							}

							if ($isMultiRM_2==="Y"){
								// if ($USER_NAME==="1949"){ echo "check $isMultiRM_2"; }
								$SqlData = "
								...
								$DtRm =  getData($conn,$SqlData);
								if ($USER_NAME==="1949") {
									// echo "isMultiRM_2 1 $isMultiRM_2 <br>";
									// echo "DtRm $DtRm<br>".$rowRsView['CYL_SYS_ID']."<br>$SqlData<br>";
								}
								if ($DtRm===""){
										$SqlData =
										"SELECT SUM (TOT) GET_DATA
										 ...
										 $DtRm =  getData($conn,$SqlData);
										 // if ($USER_NAME==="1949") {echo "isMultiRM_2 2 $isMultiRM_2 : DtRm $DtRm<br>";}
								}
								...
								if ($DtRm===""){
									/*$sqlGet = "
											 ...
											 // if ($USER_NAME==="1949") { echo "$DtRm<br>$sqlGet"; }
			...
			// if ($USER_NAME==="1949") {echo "valDt27 1 $valDt27 : isDt_RM55 $isDt_RM55 : DtRm $DtRm<br>";}
			if ($isDt_RM55!=="Y"){
				if ($rowRsView['CYL_TYPE']==="MELANGE"){
					...
					// if ($USER_NAME==="1949"){ echo "cek isDt_RM55 $isDt_RM55 ";}
					if (!empty($rowRsView['FGETMBCOST'])){
						$DtRm =	$rowRsView['FGETMBCOST'];
						// if ($USER_NAME==="1949"){ echo "cek1 $DtRm ";}
					} else {
			...
			if ($isDt_RM55==="Y"){
					...
					if ($TotPoy_Rm55 > 1){
						...
						if ($isMulti_RM==="Y"){
							...
							// if ($USER_NAME==="1949"){echo "bukan isMulti_RM $isMulti_RM<br>";}
							 $TotRecDt = 0; $DtMb = "";
							 $SqlMbDt =
							 	"
								SELECT CYCC_TOP_64_DATA_VALUE DT_VAL
								  FROM (SELECT cllp_cyl_sys_id,CLLP_LEFT_NO, cllp_seq_lvl
								          FROM cst_lvl_left_prod a
								...
								and CYCC_TOP_64_DATA_VALUE is not null
