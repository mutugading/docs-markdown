<script type='text/javascript'>
	...
	function getParameter(){
  	...
  	if (document.getElementById('HEATSET')!==null ) {
		...
		if (document.getElementById('CK_ALL')!==null ) {
      	document.getElementById('CK_ALL_S').value = document.getElementById('CK_ALL').value;
  	}
	}
	...
	function CkAllRemove(){
		//alert(prs);
		var vFormName ="<?php Print($FORM_NAME); ?>"; var vMsgConfirm="";
		getParameter();

		var vCk_all = document.getElementsByName('CK_ALL');
		var vCk_all_v = "";
		for (var i = 0, length = vCk_all.length; i < length; i++) {
				if (vCk_all[i].checked) {
						vCk_all_v = "Y";
				} else vCk_all_v = "N";
		}
		document.getElementById('CK_ALL_S').value=vCk_all_v;
		// if (vCk_all_v=="Y"){
		// 	vMsgConfirm = "Check All Data";
		// } else {
		// 	vMsgConfirm = "Un Check All Data";
		// }
		//
		// if (confirm(vMsgConfirm)){
			//document.getElementById('STSPRS').value = prs;
			document.getElementById(vFormName).submit();
		// } else
	}

	function GenerateQuotation(prs){
		//alert(prs);
		var vFormName ="<?php Print($FORM_NAME); ?>"; var vMsgConfirm="";
		getParameter();
		if (prs==="GENERATE_QUOTATION"){
			vMsgConfirm="Generate Data Selecttion ?";
			if (confirm(vMsgConfirm)){
				document.getElementById('STSPRS').value = prs;
				document.getElementById(vFormName).submit();
			}
	...
	function RemoveQuotation(prs){
		var vFormName ="<?php Print($FORM_NAME); ?>"; var vMsgConfirm="";
		getParameter();
		if (prs==="REMOVE_QUOTATION"){
			var vChoose = document.getElementsByName("REMOVE[]"); var vChoose_Dt="";
			var checked=0;
					checked= Number(checked);
			var vDelData = [];
			for (var i=0;i < vChoose.length; i++){
				if (vChoose[i].checked){
					vChoose_Dt = vChoose[i].value;
					vDelData.push(vChoose_Dt);
					checked=checked +1;
				}
			}
			if (checked  < 1 ){
				alert("Choose one of the data");
			} else {
				vMsgConfirm="Remove Data from Quotation ?";
				if (confirm(vMsgConfirm)){
					document.getElementById('STSPRS').value = prs;
					document.getElementById('SQD_CYL_LEFT_NO_DT').value = vDelData;
					document.getElementById(vFormName).submit();
				}
			}
		}
	...
	function AddQuotation(prs){
		//alert(prs);
		var vFormName ="<?php Print($FORM_NAME); ?>"; var vMsgConfirm="";
		getParameter();
		if (prs==="ADD_QUOTATION"){

			var vChoose = document.getElementsByName("CHOOSE[]"); var vChoose_Dt="";
			var checked=0;
					checked= Number(checked);
			var vDelData = [];
			for (var i=0;i < vChoose.length; i++){
			...
			if (checked  < 1 ){
				...
				vMsgConfirm="Add Data for Quotation ?";
				if (confirm(vMsgConfirm)){
					document.getElementById('STSPRS').value = prs;
					document.getElementById('SQD_CYL_LEFT_NO_DT').value = vDelData;
					document.getElementById(vFormName).submit();
