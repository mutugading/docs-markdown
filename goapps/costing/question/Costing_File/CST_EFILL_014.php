...
<?php
  ...
  include("../conOraOci.php");
	...
	echo "STSPRS $STSPRS : VIEW_LINK $VIEW_LINK</br>";
...
//echo "VIEW_LINK$VIEW_LINK </br>";
if ($VIEW_LINK!==""){
	if (substr($STSPRS,0,5) === "VIEW "){
		echo "view </br>";
		$sqlIns = "	insert into mgtapps.EFILL_LOG_ACCESS(
					...
					values (
					'123','$USER_NAME',sysdate,'View Document(Delete Document) -> $MENU_NAME -> $VIEW_LINK','$USER_NAME_DTL'
					)";
	...
	} else if (substr($STSPRS,0,7) === "DELETE ") {
		$VIEW_LINK = substr($STSPRS,7);
		$LOG_REMARKS_S = "";
	 	if(isset($_POST['LOG_REMARKS_S'])) {
	 		$LOG_REMARKS_S=$_POST['LOG_REMARKS_S'];
    	}
		$sqlIns = "	insert into mgtapps.EFILL_LOG_ACCESS(
					ELA_SYS_ID, ELA_CREATED_BY, ELA_CREATED_TIMESTAMP, ELA_DETAIL_ACCESS, ELA_USER_NAME
					)
					values (
					'123','$USER_NAME',sysdate,'View Document(Delete Document) -> $VIEW_LINK --> Remarks : $LOG_REMARKS_S','$USER_NAME_DTL'
					)";
		//echo "LOG_REMARKS_S $LOG_REMARKS_S";die();
		$result=oci_parse($conn,$sqlIns);
		if (!$result) {
		   $oerr = OCIError($conn);
		   echo "Fetch Code 1:".$oerr["message"];
		...
		echo "VIEW_LINK $VIEW_LINK";
		unlink($VIEW_LINK);

		echo "<script>alert('Delete file $VIEW_LINK Success !');</script>";
	} else if (substr($STSPRS,0,4) === "DEL "){
?>
<table style="width:80%" border="1"  align="center">
	<tr>
		<td style="width:50%" align="right"> Delete Document File&nbsp&nbsp</td>
		<td align="left"> &nbsp&nbsp<?php echo $VIEW_LINK; ?></td>
	</tr>
	<tr>
		<td style="width:50%" align="right"> Remarks&nbsp&nbsp</td>
		<td align="left">
			<input type="text" id="LOG_REMARKS" name="LOG_REMARKS">
		</td>
	</tr>
	<tr>
		<td align="center" colspan="2">
<?php $deletePrs = "Process('DELETE D:/XAMPP/htdocs/webapps/$VIEW_LINK')" ?>
			<input type="button" value="DELETE" class="BUTTON btn_delete" onclick="<?php echo $deletePrs;?>">
			<input type="button" value="CANCEL" class="BUTTON btn_cancel" onclick="Process('')">
		</td>
	</tr>
</table>
<?php
...
<?php
	...
	if ($EFILL_ADMIN==="ADMIN"){
		$sqlList =
					...
					where ETD_SYS_ID = 'CARPET PRODUCT'
					";
	} else {
		$sqlList =
					"select 	ETD_SYS_ID
...
<?php
		...
		<td style="width:<?php echo $withDisp; ?>%" align="center">
			<input 	type="button" value="Del" onclick="<?php echo "Process('DEL $printLink')"; ?>" class="BUTTON btn_delete" >
			</br>
...
<input type="hidden" name="LOG_REMARKS_S" id="LOG_REMARKS_S">
<script type='text/javascript'>
	function Process(prs){
		...
		if (document.getElementById('ETD_SYS_ID')!==null ) {
			ETD_SYS_ID_S=document.getElementById('ETD_SYS_ID');
		...
		var ETD_SYS_ID_S_OLD="";
		if (document.getElementById('ETD_SYS_ID_S_OLD')!==null ) {
			ETD_SYS_ID_S_OLD=document.getElementById('ETD_SYS_ID_S_OLD').value;
		}
    	...
    	if (prs.substr(0,5)==="VIEW "){
    		vConfirm = confirm("View Data FILE " + prs.substr(5) + " ?");
			if (vConfirm === true){
				document.getElementById('VIEW_LINK').value = prs.substr(5);
			}
    	}
    	if (prs.substr(0,4)==="DEL "){
			vConfirm = confirm("Delete Data FILE " + prs.substr(5) + " ?");
			if (vConfirm === true){
				document.getElementById('VIEW_LINK').value = prs.substr(4);
			}
    	}

    	if (prs.substr(0,7)==="DELETE "){
    		var linkFile = "<?php Print($VIEW_LINK); ?>";
    		document.getElementById('VIEW_LINK').value = linkFile;
			if (document.getElementById("LOG_REMARKS").value === ""){
				document.getElementById('STSPRS').value = "DEL "+linkFile;
				alert("Remarks can't null");
			}
			document.getElementById("LOG_REMARKS_S").value = document.getElementById("LOG_REMARKS").value;
    	}
