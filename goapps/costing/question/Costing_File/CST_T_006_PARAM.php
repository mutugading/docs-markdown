...
<table style="width:100%">
	<th align="center" colspan="2">CHOOSE DATA</th>
	<tr>
<!-- Product Type -->
		<td align="center">
			Product Type
<?php
		...
		$sqlSlct = " SELECT DISTINCT CYL_TYPE from mgtapps.cst_yarn_left where CYL_IS_VALID_PRD = 'Y' and CYL_TYPE = 'SUPERBA'";
		//echo $sqlSlct;
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"CUSTOMER",$WHERE_PARAM);
	//echo "$sqlSlct </br>";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
		...
		$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"CYL_SHADE_NAME",$WHERE_PARAM)." order by CYL_SHADE_NAME ";
		// if ($USER_NAME==="1949"){
		// 		echo $sqlSlct;
		// }
		$rsSlct = oci_parse($conn,$sqlSlct);
...
<!-- Denier -->
		<td  <?php echo getStyle($font,"center"); ?>>
			Denier  </br>
<?php
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"DENIER",$WHERE_PARAM)." order by to_number(DENIER) ";
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"FILAMENT",$WHERE_PARAM)." order by to_number(FILAMENT) ";
	//echo "sqlSlct $sqlSlct</br>";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"INTERMINGLING",$WHERE_PARAM)." order by INTERMINGLING ";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"HEATSET",$WHERE_PARAM)." order by HEATSET ";
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"CROSS_SECTION",$WHERE_PARAM)." order by CROSS_SECTION ";
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
...
<?php
	...
	$sqlSlct =sqlSlctTypeBaseSuperba($conn,$sqlData,$CYL_TYPE_S,"LUSTURE",$WHERE_PARAM);
	//echo $sqlSlct;
	$rsSlct = oci_parse($conn,$sqlSlct);
	oci_execute ($rsSlct);
?>
