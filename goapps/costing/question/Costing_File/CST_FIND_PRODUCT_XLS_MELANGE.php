...
$deepMaterial =  getData($conn,$SqlData);
if ($deepMaterial==="0"){
  $SqlData = getSqlDeepMelangeUneven($P_CYL_SYS_ID_DTL);
  $deepMaterial =  getData($conn,$SqlData);
}
//if ($P_CYL_SYS_ID_DTL==="2026041643132"){ echo "SqlData<br> $SqlData<br> deepMaterial $deepMaterial"; die(); }
?>
<html>
<head>
</head>
...
while ($row1Data = oci_fetch_array ($rs1Data, OCI_BOTH)) {
  ...
  //if ($row1Data['CYCRL_SEQ_NO']==="2"){if ($P_CYL_SYS_ID_DTL==="2026041643132"){die();}}
  ...
  while ( $totRM > $lpDt) {
    ...
    if (intval($row1Data['CYCRL_SEQ_NO'])>=36 && intval($row1Data['CYCRL_SEQ_NO'])<=52){
        $ConvCostCaptive[$lpDt] = (float)$ConvCostCaptive[$lpDt] + (float)$dtCek;//$ConvCostCaptive[$lpDt]=$ConvCostCaptive[$lpDt] + $dtCek;
        $ConvCostDel[$lpDt]= (float)$ConvCostDel[$lpDt] + (float)$dtCek;//$ConvCostDel[$lpDt]=$ConvCostDel[$lpDt] + $dtCek;
    }
